...
class ParticleSystem:
    def __init__(self, res):
        ...
        self.particle_radius = 0.08  # particle radius
        self.particle_diameter = 2 * self.particle_radius
        self.support_radius = self.particle_radius * 3.0  # support radius
        self.m_V = 0.7854 * self.particle_diameter ** self.dim
    ...
    def add_boundary(self, boundaryColor):
        delta = self.grid_size
        if self.dim == 2:
            self.add_cube(lower_corner=[0, 0],
                        cube_size=[self.bound[0], delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[0, self.bound[1] - delta],
                        cube_size=[self.bound[0], delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[0, delta],
                        cube_size=[delta, self.bound[1] - 2 * delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[self.bound[0] - delta, delta],
                        cube_size=[delta, self.bound[1] - 2 * delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
        else:
            self.add_cube(lower_corner=[0, 0, 0],
                        cube_size=[self.bound[0], self.bound[1], delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[0, 0, self.bound[2] - delta],
                        cube_size=[self.bound[0], self.bound[1], delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[0, 0, delta],
                        cube_size=[self.bound[0], delta, self.bound[2] - 2 * delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[0, self.bound[1] - delta, delta],
                        cube_size=[self.bound[0], delta, self.bound[2] - 2 * delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[0, delta, delta],
                        cube_size=[delta, self.bound[1] - 2 * delta, self.bound[2] - 2 * delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
            self.add_cube(lower_corner=[self.bound[0] - delta, delta, delta],
                        cube_size=[delta, self.bound[1] - 2 * delta, self.bound[2] - 2 * delta],
                        density=1000.0,
                        color=boundaryColor,
                        material=0)
